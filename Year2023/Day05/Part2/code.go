...
func Calculate(input string) int {
	...
	for _, seedString := range seedStrings {
		...
		seedParams = append(seedParams, seed)
	}

	// 0 is the seedParams row. 1 is the separator line
	rows = rows[2:]
	...
	for i := 0; i < len(seedParams); i = i + 2 {
		for j := 0; j < seedParams[i+1]; j++ {
			startingSeed := seedParams[i] + j
			destinationLocation := followTrail(almanacMaps, "seed", startingSeed)

			if (i == 0 && j == 0) || (destinationLocation < lowestLocationNumber) {
